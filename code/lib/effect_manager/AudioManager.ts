...
export class Audio {
    ...
    private async awaitLoad(url: string) {
        ...
        if (loader ) {
            return new Promise<cc.AudioClip>((resolve, reject) => {
                loader.load(url, (err: Error, clip: cc.AudioClip) => {
                    if (err) {
                        Debug.error('音频加载失败');
                        this._err = err;
                        reject('rejected');
        ...
        for (let i: number = 0; i < this._audioList.length; ++i) {
            ...
            if (audioPro.bgm) {
                if ((audioPro.audio as IAudioBGM).superpose) {
                    audioID = cc.audioEngine.play(audioPro.clip, audioPro.audio.loop, audioPro.audio.volume);
                }
                else {
                    audioID = cc.audioEngine.playMusic(audioPro.clip, audioPro.audio.loop);
                    cc.audioEngine.setVolume(audioID, audioPro.audio.volume);
                }
            }
            else {
                audioID = cc.audioEngine.playEffect(audioPro.clip, audioPro.audio.loop);
                cc.audioEngine.setVolume(audioID, audioPro.audio.volume);
            }
            audioPro.audioID = audioID;
            Audio._audioPool.set(key, audioPro);
            //设置音频结束后的回调
            cc.audioEngine.setFinishCallback(audioID, () => this.onFinishCallback(audioPro, audioID));
            for (let e of audioPro.callbacks) {
                if (e.type === 'play') {
                    SAFE_CALLBACK(e.call, cc.audioEngine.getCurrentTime(audioID));
                }
            }
        }
    }
    ...
    private playAudioInOrder(resolves: cck_audio_saudio_resolved_type[]) {
        ...
        Audio._audioPool.set(key, audioPro);
        //设置音频结束后的回调
        cc.audioEngine.setFinishCallback(audioID, () => this.onFinishCallbackInOrder(audioPro, audioID));
