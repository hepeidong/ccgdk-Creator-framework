...
import { App, Res } from "../cck";

const RESOURCES = cc.resources.name;
...
export abstract class AssetFactory {
    ...
    private _prefabAsset: cc.Prefab;
    private _assets: Map<string, cc.Asset[]>;
    ...
    public setSpriteFrame(target: cc.Node|cc.Sprite, filename: string) {
        if (target instanceof cc.Node) {
            ...
            if (sprite) {
                sprite.spriteFrame = this.getGameAsset(filename, cc.SpriteFrame);
            }
        }
        else if (target instanceof cc.Sprite) {
            target.spriteFrame = this.getGameAsset(filename, cc.SpriteFrame);
        }
    }
    ...
    private loadPrefab() {
        return new Promise<cc.Prefab>((resolve, reject) => {
            const prefab = this._loader.get<cc.Prefab>(this._assetPath, cc.Prefab);
            if (prefab) {
                this._finish++;
                SAFE_CALLBACK(this._onProgress, this._finish / this._total);
                resolve(prefab);
                ...
                this._loader.load<cc.Prefab>(this._assetPath, cc.Prefab, (finish, total) => {
    ...
    protected addAssets(assets: cc.Asset | cc.Asset[]) {
    ...
    private pushToAssets(asset: cc.Asset) {
