...
type LoadRemoteArgs = {
    url: string, options: Record<string, any>, onComplete: (err: Error, asset: cc.Asset) => void
} 
...
export class Res {
    ...
    public static get loader() {
        if (!this._loader) {
            this._loader = new Loader(cc.resources);
        }
        return this._loader;
    }
    ...
    public static removeLoader(name: string, release: boolean = true) {
        if (this._loaderPool.has(name)) {
            ...
            release && loader.gc();
            cc.assetManager.removeBundle(loader.bundle);
     ...
     */
    public static setRemoteSpriteFrame(target: cc.Node|cc.Sprite, remoteUrl: string) {
     ...
     */
    public static loadRemote<T extends cc.Asset>(url: string, options: Record<string, any>, onComplete: (err: Error, asset: T) => void): void;
    public static loadRemote<T extends cc.Asset>(url: string, onComplete: (err: Error, asset: T) => void): void;
    public static loadRemote<T extends cc.Asset>(url: string, options: Record<string, any>): void;
    public static loadRemote<T extends cc.Asset>(url: string): void;	
    public static loadRemote()	 {
        const args = makeLoadRemoteArgs.apply(null, arguments) as LoadRemoteArgs;
        if (args.options) {
            cc.assetManager.loadRemote(args.url, args.options, (err, asset) => {
                this.loadComplete(err, asset, args.onComplete);
            });
        }
        else {
            cc.assetManager.loadRemote(args.url, (err, asset) => {
                this.loadComplete(err, asset, args.onComplete);
            });
        }
    }
    ...
    private static loadComplete(err: Error, asset: cc.Asset, onComplete: (error: Error, asset: cc.Asset) => void) {
        if (!err) {
    ...
    private static loadAssetSync(path: string, type: typeof cc.Asset) {
