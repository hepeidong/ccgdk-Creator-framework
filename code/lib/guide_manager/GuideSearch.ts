...
/**检索引导目标 */
 export class GuideSearch {
    ...
    private _lightTargets: Map<number, cc.Node[]>;             //高亮的目标节点
    ...
    public getLightTargets(): Map<number, cc.Node[]> { return this._lightTargets; }
    ...
    public searchLightTarget(guideId: number) {
        ...
        if (panel && panel.scope === Guide.Scope.PARENT_PANEL) {
            this.addLightTarget(guideId, this._guidePanels.get(uiId).target);
        }
        else {
            uiId = this._guideFile.get(guideId).uiId;
            this.traversalChild(this._guidePanels.get(uiId).target, (target: cc.Node) => {
    ...
    public addLightTarget(guideId: number, target: cc.Node) {
        if (!target) {
        ...
        if (!this._lightTargets.has(guideId)) {
            ...
            let nodes: cc.Node[] = this._lightTargets.get(guideId);
    ...
    private searchGuideTarget(target: cc.Node) {
        this.traversalChild(target, this.storageGuideTarget.bind(this));
    }

    private traversalChild(parent: cc.Node, callback: (target: cc.Node) => void) {
        for (let i: number = 0; i < parent.childrenCount; ++i) {
            //设置节点层级,以便引导完之后恢复节点原本的层级
            parent.children[i].zIndex = parent.children[i].zIndex === 0 ? this._zIndex++ : parent.children[i].zIndex;
            
            callback && callback(parent.children[i]);
            if (parent.children[i] && parent.children[i].childrenCount > 0) {
    ...
    //存储引导目标
    private storageGuideTarget(target: cc.Node) {
    ...
    private storageLightTarget(guideId: number, target: cc.Node) {
        let targetID: WidgetID = target.getComponent(WidgetID);
        if (!targetID) {
            return;
        }
