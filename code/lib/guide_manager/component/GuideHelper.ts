...
export default class GuideHelper extends cc.Component {
    ...
    @property(cc.Node)
    private _guideBlockInput: cc.Node = null;
    ...
    onLoad() {
        ...
        if (!CC_EDITOR) {
            this.onGuideOver();
            Guide.guideManager.setFingerSpeed(this.fingerSpeed);
            Guide.guideManager.addGuideMaskAndLayer(this._guideMask, this._guideLayer);
            Guide.guideManager.setGuideStart(this.onStartGuide, this);
            Guide.guideManager.setGuideOver(this.onGuideOver, this);
            Guide.guideManager.setGuideEnd(this.onGuideOver, this);
    ...
    start() {
        Guide.guideManager.guideLaunch();
    }

    createNode() {
        if (!this._guideMask) {
            let newNode: cc.Node = new cc.Node('guideMask');
        ...
        if (!this._guideLayer) {
            let newNode: cc.Node = new cc.Node('guideLayer');
            this._guideLayer = newNode;
            this.node.addChild(this._guideLayer);
        }
        if (!this._guideBlockInput) {
            let newNode: cc.Node = new cc.Node('guideBlockInput');
            newNode.addComponent(AdapterWidget).size = true;
            newNode.addComponent(cc.BlockInputEvents);
    ...
    onStartGuide(guideId: number) {
        ...
        if (status > -1) {
            ...
            Debug.log('当前引导', Guide.guideManager.guideInfo);
    ...
    private executeGuide(guideId: number) {
        if (Guide.guideManager.isGuiding) {
            let guide: GuideFinger | GuideText | GuideDialogue;
            Guide.guideManager.setAgainExecute(false);
            if (Guide.guideManager.guideType === Guide.GuideType.DIALOGUE) {
                ...
                guide = this.guideDialogue.getComponent(GuideDialogue);
            }
            else if (Guide.guideManager.guideType === Guide.GuideType.FINGER) {
                Debug.log('[GuideHelper] onStartGuide 手指引导');
                if (Guide.guideManager.searchLightTarget(guideId)) {
                    this._guideMask && (this._guideMask.active = true);
                    this.guideFinger && (this.guideFinger.active = true);
                    guide = this.guideFinger.getComponent(GuideFinger);
            ...
            else if (Guide.guideManager.guideType === Guide.GuideType.TEXT) {
                Debug.log('[GuideHelper] onStartGuide 文本引导');
                if (Guide.guideManager.searchLightTarget(guideId)) {
                    this._guideMask && (this._guideMask.active = true);
                    this.guideText && (this.guideText.active = true);
                    guide = this.guideText.getComponent(GuideText);
                }
                else {
                    Debug.log('没有高亮节点', guideId);
                    Guide.guideManager.setAgainExecute(true);
    ...
    private getViewStatus(guideId: number) {
        let status: number = -1;
        let guideInfo = Guide.guideManager.guideFile.get(guideId);
