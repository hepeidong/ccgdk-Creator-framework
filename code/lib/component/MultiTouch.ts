...
export default class MultiTouch extends cc.Component {
    ...
    public moveToTarget(origGrNums: number[], tarGrNums: number[], durat: number, recover: boolean = true): boolean {
        ...
        for (let i: number = 0; i < tarGrNums.length; ++i) {
            ...
            const worldPos = this.targetArea.target.convertToWorldSpaceAR(cc.v2(this.tarGrooves[tarGrNums[i] - 1].x, this.tarGrooves[tarGrNums[i] - 1].y));
            const position = origBalls[i].ball.parent.convertToNodeSpaceAR(worldPos);
    ...
    private registerEvent() {
        for (let i: number = 0; i < this.ballList.length; ++i) {
            this.ballList[i].ball.on(cc.Node.EventType.MOUSE_ENTER, () => this.onMouseEnter(i), this);
            this.ballList[i].ball.on(cc.Node.EventType.MOUSE_LEAVE, () => this.onMouseLeave(i), this);
            this.ballList[i].ball.on(cc.Node.EventType.TOUCH_START, (evt) => {
                let data = {
                    content: {
                        i: i,
                        x: evt.getLocationX(),
            ...
            this.ballList[i].ball.on(cc.Node.EventType.TOUCH_MOVE, (evt) => {
                let data = {
                    content: {
                        i: i,
                        x: evt.getLocationX(),
            ...
            this.ballList[i].ball.on(cc.Node.EventType.TOUCH_END, () => {
                let data = {
                    content: {
                        i: i
                    }
            ...
            this.ballList[i].ball.on(cc.Node.EventType.TOUCH_CANCEL, () => {
                let data = {
                    content: {
                        i: i
                    }
    ...
    private getMinDisOfIndex(targetList: any[], ball: cc.Node, parent: cc.Node, callback: (index: number) => boolean): number {
        ...
        for (let i: number = 0; i < targetList.length; ++i) {
            ...
            if (targetList[i].position) {
                dis = Utils.MathUtil.distance(cc.v3(position.x, position.y), targetList[i].position);
            }
            else {
                dis = Utils.MathUtil.distance(cc.v3(position.x, position.y), targetList[i]);
            }
            min = {dis: dis, index: i};
