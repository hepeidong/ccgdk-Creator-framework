...
export class AdManager {
    ...
    public showGridAd(target: cc.Node, adUnitId: string, adTheme: 'white'|'black', gridCount: 5|8) {
        if (!this._gridAd) {
            let position: cc.Vec2 = Utils.EngineUtil.convertToWechatSpace(target);
            let size: cc.Size = Utils.EngineUtil.convertToWechatSize(target);
    ...
    public showCustomAd(target: cc.Node, adUnitId: string, adIntervals: number, hided: boolean): CustomAdInterface {
        if (!this._customAd) {
    ...
    private createGridAd(target: cc.Node, obj: { adUnitId: string, adIntervals?: number, 
