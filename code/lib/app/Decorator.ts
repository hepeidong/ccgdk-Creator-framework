...
const setProp = setup("propKeys");
const setModel = setup("_data");
const setHttpUrl = setup("URL");
const setHttpMethod = setup("METHOD");
...
function cckclass() {
    ...
    if (typeof arguments[0] === "string") {
        ...
        return function (constructor: Function) {
            if (cc.js.isChildClassOf(constructor, parentMap.gameWorld)) {
                GAME_NAME.name = className;
            }
            else if (cc.js.isChildClassOf(constructor, parentMap.system)) {
                //ECS中的System的子类
                addClassName(systemClassNames, className);
            }
            else if (cc.js.isChildClassOf(constructor, parentMap.redDot)) {
