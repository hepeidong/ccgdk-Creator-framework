...
export class WindowManager {
    ...
    constructor() {
        ...
        this._managers[UI.Type.ROOT] = new RootManager(true, this._windowLayer);
        this._managers[UI.Type.DIALOG] = new DialoglManager(true, this._windowLayer);
        this._managers[UI.Type.ACTIVITY] = new ActivityManager(true, this._windowLayer);
        this._managers[UI.Type.TOAST] = new ToastManager(false, this._windowLayer);
        this._managers[UI.Type.TOP] = new TopManager(false, this._windowLayer);
    }
    ...
    private retrieveView(accessId: string) {
        ...
        if (this.has(accessId)) {
            ...
            const classRef = cc.js.getClassByName(accessId) as Constructor;
    ...
    public disableBubble(disable: boolean) {
        const manager = this._managers[UI.Type.TOAST] as ToastManager;
     ...
     */
     public push(priority: number, accessId: string) {
        ...
        if (view.getViewType() === UI.Type.ACTIVITY) {
            const manager = this._managers[UI.Type.ACTIVITY] as ActivityManager;
    ...
    public exitView(view: IWindowBase) {
        ...
        if (type === UI.Type.TOAST) {
            //把Toast类型UI放回对象池
            const manager = this._managers[type] as ToastManager;
            manager.put(view);
        }
        else if (type === UI.Type.ACTIVITY) {
            //按照出栈方式以此弹出活动面板
            this.pop();
        }
    }
     ...
     * @param canvas 
     */
    public initWindowLayer(canvas: cc.Node, hasTouchEffect: boolean) {
    ...
    public initTouchEffectAsset(prefab: cc.Prefab) {
        this._touchEffect.initEffectAsset(prefab);
    }

    public addToTop(target: cc.Node) {
        this._windowLayer.addToTop(target);
    }

    public shiftMask() {
        for (let i: number = UI.Type.TOP; i >= 1; --i) {
            let win: IWindowBase = this._managers[i].getView();
            if (win) {
                App.game.addViewMaskTo(win.view.node.parent, win, win.view.node.zIndex - 1);
                return;
            }
        }
        App.game.delViewMask();
    }
