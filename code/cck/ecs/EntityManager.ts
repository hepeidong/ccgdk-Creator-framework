...
import { EntityChange, EntityReleased, IEntities, IBaseEntity, IEntityArchetype, IEntityManager } from "../lib.cck";
import { tools } from "../tools";

/**
 * 实体服务类, 管理运行的实体, 实体对象, 运行的组, 组对象
 */
export class EntityManager implements IEntityManager<IBaseEntity> {
    private _entitys: IEntities;           //存放所有正在运行的实体的表
    private _componentNum: {};               //组件枚举
    private _totalComponent: number;         //组件最大个数
    private _reusableEntitys: tools.ObjectPool<IBaseEntity>;             //实体对象池
    private _archetypeChunk: ArchetypeChunk<IBaseEntity>;
    ...
    public createEntity(name: string) {
        ...
        let entity: IBaseEntity = this._reusableEntitys.size() > 0 ? this._reusableEntitys.get() : 
    ...
    public destroyEntity(entity: IBaseEntity) {
        if (!entity.enabled) {
        ...
        let group: IEntityArchetype<IBaseEntity> = this._archetypeChunk.archetypes[entity.groupId];
        group.removeEntityFromChunk(entity);
    }

    public destroyEntityAll() {
        for (let k in this._entitys) {
            this.destroyEntity(this._entitys[k] as IBaseEntity);
        }
    }
    ...
    public getEntities(): IBaseEntity[] {
        return this._archetypeChunk.getEntities();
    }

    private updateGroupByComponentAdded(entity: IBaseEntity, index: number) {
    ...
    private updateGroupByComponentRemoved(entity: IBaseEntity, index: number) {
    ...
    private onEntityReleased(entity: IBaseEntity) {
        entity.onEntityReleased.remove(this.onEntityReleased);
        entity.destroyEntity();
    }

    private onEntityDestroyed(entity: IBaseEntity) {
        if (entity.enabled) {
